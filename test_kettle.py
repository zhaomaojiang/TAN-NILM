...
app_name = 'kettle'  # todo: 'washingmachine', 'dishwasher', 'microwave', 'fridge', 'kettle'
if data_set_name == 'REDD':
    batch_si = 32
else:
    batch_si = 64
folder_name = './data/REFIT_dataset/'+'refit_64ket_sam8_sen0'+'.pickle'  # todo:
stri = was_appliance[app_name]['stride']

checkpoint = torch.load('check_'+app_name+'/spexlast279.pt')

model = TAN_Plus(L1=3, N=14, B=3, O=14, P=14, Q=3, num_spks=5, spk_embed_dim=14, Head=2, causal=False).cuda()
